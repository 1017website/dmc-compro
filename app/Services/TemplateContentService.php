...
HTML;
    ...
    private function mediaCollectionsScript(array $settings): string
    {
        ...
        return <<<HTML
<style>
.gallery-grid.has-dynamic-items{grid-template-columns:repeat(auto-fit,minmax(min(260px,100%),1fr));grid-template-rows:none;grid-auto-rows:310px}.gallery-grid.has-dynamic-items .gallery-item{grid-column:auto}@media (min-width:961px){.portfolio-mockup-grid.has-dynamic-items .portfolio-mockup-card,.portfolio-mockup-grid.has-dynamic-items .portfolio-mockup-card.is-featured{grid-column:span var(--dmc-card-span,3)}}.portfolio-thumbnail video{width:100%;height:100%;object-fit:cover;transition:transform 600ms cubic-bezier(.2,.8,.2,1);pointer-events:none}.portfolio-thumbnail:hover video{transform:scale(1.035)}.media-collection-empty{grid-column:1/-1;padding:42px;border:1px dashed rgba(0,0,0,.2);color:#777;text-align:center}.portfolio-mockup-grid .media-collection-empty{color:rgba(255,255,255,.64);background:#171717}
</style>
<script>
(function(){
 'use strict';
